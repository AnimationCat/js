...
class LineAPI {
  ...
  async _AddFriendByMid(Mid) {
    return this._client.findAndAddContactsByMid(Mid);
  }

  async new_sendMessage(to, text) {
    let txt = new Message();
    txt.to = operation.param1;
    txt.text = text;
    return this._client.sendMessage(0, txt);
  }
