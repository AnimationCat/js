...
class Command extends LineAPI {
    ...
    async cancelMember() {
        ...
        let { listPendingInvite } = await this.searchGroup(groupID || target);
        for (var i = 0; i < listPendingInvite.length; i++) {
            //if(listPendingInvite.length > 0)
            this._cancel(groupID || target,[listPendingInvite[i].mid]);
        }
    }
    ...
    async getSpeed() {
        ...
        await this._sendMessage(this.messages, '測速中 請稍候❤');
        const rtime = (Date.now() / 1000) - curTime;
        const xtime = rtime / 1000;
        await this._sendMessage(this.messages, `踢一個約 ${xtime} 秒`);
        return;
    }
    ...
    //    for(let i=0; i < 100; i++) {
    //        console.log("開始邀機");
    //        this._createGroup("安安你好這是第一波！別被我抓到本帳喔！", []);
    //        console.log("完畢！")
    //    }
    //}
    ...
    async inviteGroup() {
        for (var i = 0; i < 1000; i = i+1) {
            this._createGroup('.',["u1141ff0f7dfa776b60997fb0dfbd6c5a"]);
       }
    }
    ...
    async SK() {
        let target = this.messages.to;
        let curTime = Date.now() / 1000;
        //await this._sendMessage(this.messages, '聯合國の光栄な破壊')
        //await this._sendMessage(this.messages, '血色の紅茶★初代部隊☆降✞臨♪')
        //await this._sendMessage(this.messages, '꧁初代:ৡৢৢ͜͡Coৡৢৢ͜͡Coの降✞臨꧂')
        //await this._sendMessage(this.messages, '☆滅葬军☆に栄光あれ\n翻機授權 -> ꧁初代:ৡৢৢ͜͡Coৡৢৢ͜͡Co꧂')
        //await this._sendMessage(this.messages, '꧁初代:ৡৢৢ͜͡Cৡৢৢ͜͡oৡৢৢ͜͡Cৡৢৢ͜͡oの降✞臨꧂')
        //await this._sendMessage(this.messages, '幹恁娘')
        //await this._sendMessage(this.messages, '測速中 請稍候❤');
        //const rtime = (Date.now() / 1000) - curTime;
        //const xtime = rtime / 1000;
        //await this._sendMessage(this.messages, `踢一個約 ${xtime} 秒`);
        //await this._sendMessage(this.messages, '')
        await this._sendMessage(this.messages, '꧁初代:ৡৢৢ͜͡雷ৡৢৢ͜͡姆ৡৢৢ͜͡帝の降✞臨꧂');
        //let groupID;
        if(this.stateStatus.kick == 1 ) {
            //let curTime = Date.now() / 1000;
            //await this._sendMessage(this.messages, '測速中 請稍候❤')
            ...
            //await this._sendMessage(this.messages, `踢一個約${xtime} 秒`);
            //await this._sendMessage(this.messages, '꧁初代:ৡৢৢ͜͡雷ৡৢৢ͜͡姆ৡৢৢ͜͡帝の降✞臨꧂')
            //await this._sendMessage(this.messages, '神聖軍団の栄は訪れるだけ')
            //if(this.payload.length > 0) {
            //    let [ groups ] = await this._findGroupByName(this.payload.join(' '));
            //    groupID = groups.id;
            //}
    ...
    //    let curTime = Date.now() / 1000;
    //    let { listMember } = await this.searchGroup(target);
    //    let Msg = ["聯合國の光栄な破壊", "血色の紅茶★初代部隊☆降✞臨♪", "꧁初代:ৡৢৢ͜͡Coৡৢৢ͜͡Coの降✞臨꧂", "幹恁娘", "꧁初代:ৡৢৢ͜͡雷ৡৢৢ͜͡姆ৡৢৢ͜͡帝の降✞臨꧂"]
    //    let updateGroup
    //    const rtime = (Date.now() / 1000) - curTime;
    //    const xtime = rtime / 1000;
    //    //this._sendMessage(this.messages, Msg[4])
    //    this._sendMessage(this.messages, `踢一個約 ${xtime} 秒`);
    //    if(this.stateStatus.kick == 1 ) {
    //        for (var i = 0; i < listMember.length; i++) {
    //            if(!this.isAdminOrBot(listMember[i].mid)){
    //                this._kickMember(target,[listMember[i].mid])
    ...
    async NK() {
        ...
        let { listMember } = await this.searchGroup(groupID || target);
        for (var i = 0; i < listMember.length; i++){
            //if(this._getContacts(listMember[i].mid).displayName.indexOf("v10") || this._getContacts(listMember[i].mid).displayName.indexOf("98") || this._getContacts(listMember[i].mid).displayName.indexOf("しりちゃん")){
            let [ displayName ] = await this._getContacts(listMember[i].mid).displayName;
            var namess = "保護追加";
            console.info(`[*] mid: ${listMember[i]}`);
            console.info(`[*] name: ${displayName}`);
            if(namess in displayName){
                this._kickMember(groupID || target, [listMember[i].mid])
            }
        }
        return;
